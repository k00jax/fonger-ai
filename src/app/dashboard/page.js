...
function SwarmDashboard() {
  ...
  if (!state)
    return (
      <div className="min-h-screen bg-[#08081a] flex items-center justify-center relative z-10">
  ...
  return (
    <div className="min-h-screen bg-[#08081a] text-[#33ff33] font-mono relative">
      {/* Scanlines */}
      <div
      ...
      {/* Top bar */}
      <div className="fixed top-0 left-0 right-0 z-40 flex items-center gap-4 px-4 py-2.5 bg-[#08081aee] backdrop-blur-sm border-b border-[#33ff3308] text-xs">
        <b className="text-[#33ff33] text-sm tracking-wide">
          THE TRIVANCE · SWARM
        </b>
      ...
      {/* Main view */}
      <div className="pt-16 px-4 pb-12 relative z-10">
        {/* Department rings */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          {depts.map((dept) => {
            const deptAgents = agents.filter((a) => a.dept === dept);
            const deptActive = deptAgents.filter(
              (a) => a.status === 'running' || a.status === 'busy'
            );
            ...
            return (
              <div
                key={dept}
                className="bg-[#0d0d2a] border border-[#33ff3308] rounded-xl p-3.5 transition-all duration-300 hover:border-[#33ff3315] hover:bg-[#0f0f30]"
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="ml-auto text-[10px] text-gray-600">
                    {deptActive.length}/{deptAgents.length}
                  </span>
                </div>
                <div className="space-y-1">
                  {deptAgents.map((a) => {
                    ...
                    return (
                      <div
                        key={a.name}
                        className={`flex items-center justify-between text-[10px] px-2 py-1.5 rounded-lg transition-colors duration-300 ${
                          isRunning
                            ? 'bg-[#33ff3306] border border-[#33ff3312]'
                            : 'text-gray-600'
                        }`}
                      >
                        ...
                        <span className="text-gray-600 truncate ml-2 max-w-[80px]">
                          {a.task || 'idle'}
                        </span>
                      </div>
                    );
        ...
        {/* Events feed */}
        <div className="mt-4 bg-[#0d0d2a] border border-[#33ff3308] rounded-xl p-4">
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {(state.events || [])
              ...
              .map((ev, i) => (
                ...
                >
                  <span className="text-gray-600 w-10 flex-none">
                    {ev.time}
                  </span>
                  <span
                    className={`w-1.5 h-1.5 rounded-full flex-none ${
            ...
            {(!state.events || state.events.length === 0) && (
              <p className="text-gray-600 text-[10px] py-1">
